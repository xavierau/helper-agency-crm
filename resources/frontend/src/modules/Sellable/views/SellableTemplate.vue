<template>
    <div class="SellableTemplate">
        <div v-if="product">
            <h1>Print template for {{ product.name }}</h1>
...
export default {
    ...
    created() {
        client.get(endpoints.show(this.$route.params.product_id))
            .then(({data}) => this.product = data)
            .catch(error => this.$toasted.error(error.message))

        client.get(endpoints.load_sellable_template(this.$route.params.product_id))
